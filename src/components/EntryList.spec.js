...
describe('EntryList', () => {
  it('maps over the content array and converts them into journal entries', () => {
    render(
      <div>
        {content.map(item => (
          <JournalEntry key={item.id} item={item} title={item.title} />
        ))}
      </div>
    );

    const journalEntry = screen.getByTitle('On the road to Mandalay');
    expect(journalEntry.date).toBe('08-11-2016');
  });
});
